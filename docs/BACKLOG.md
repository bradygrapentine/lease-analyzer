...
| Axis         | Value                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | Gate                                         |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------- |
| Source       | ~186 non-test files + ~198 test files; 7 Playwright specs in `tests/e2e/`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | `find app/src -name '*.ts' -o -name '*.tsx'` |
| Tests        | ~1744 passing (app, post-Wave-56) + 8 in `cli/` + 6 e2e (chromium) + 1 gated real-model spec                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    | `npm test`                                   |
| Coverage     | thresholds **97 / 90 / 93 / 97** (post-Wave-57 ratchet); see `docs/TESTING.md` for current actuals                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `npm run test:coverage`                      |
...
| App.tsx      | 23 hooks under `src/App/use*.ts` (Wave 7-D + Waves 17-21 + Phase-20 follow-ups); sub-components: `AppHeader`, `AppRedlinePane`, `AppCurrentPane`, `AppLibraryAndPacksPane`, `AppAuditPane`, `AppSettingsPane` (post Wave 51-A; `AppFooterControls` retired); App.tsx **~670 lines**                                                                                                                                                                                                                                                                                                              | —                                            |
...
### Phase 20 closeout — extra polish landed in Waves 51–56

Phase 20 above covers the IA + new-component shipping list. Waves
51–56 layered design-fidelity, vocabulary, and motion / empty-state /
narrow-viewport polish on top of that base. Logged here so future
sessions know it's done.

- [x] Wave 51 — marginalia integration (PRs #185–#188 plus closeouts).
      Slim header, Settings tab, MarginaliaReader + ReaderPdfToggle +
      FindingRail, FindingDetailModal, restyled Findings/Portfolio/
      Redline/Audit panes.
- [x] Wave 52 — bolder calibration sweep across MarginaliaReader,
      FindingsPanel, FindingDetailModal, UploadView, PortfolioPanel,
      and AppRedlinePane.
- [x] Wave 53 — design fidelity: Source Serif 4 woff2 fix, severity
      glyph family (▲ ◆ ● ■), word-level `.diff-strike` / `.diff-add`,
      audit chain ribbon, viewport-fill Current layout, cards-first
      Portfolio with matrix toggle.
- [x] Wave 54 — polish residue: clear-all Negative Red, kill the
      native file-chooser leak in UploadView, empty-home privacy line
      (PRs #216 / #217 / #218).
- [x] Wave 55 — renter-friendly vocabulary pass across
      AuditLogPanel / AppRedlinePane / SideLetterPanel /
      AppLibraryAndPacksPaneSections / UploadView (PR #219).
- [x] Wave 56 — UX enhancement triple: motion vocabulary (#220),
      proper empty + error states (#221), narrow-viewport adapt for
      AppHeader + MarginaliaReader (#222).
