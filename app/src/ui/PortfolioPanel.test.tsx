...
// Wave 53-E — Cards is the default view. Matrix-specific tests switch
// via the view toggle.
async function gotoMatrix(): Promise<void> {
...
describe('PortfolioPanel', () => {
  ...
  it('renders a row per lease and a column per unique rule id', async () => {
    const leases = [meta({ id: 'a', name: 'A.pdf' }), meta({ id: 'b', name: 'B.pdf' })];
    const findingsByLease = new Map<string, Finding[]>([
      ['a', [f({ ruleId: 'late-fees', severity: 'high' })]],
      ['b', [f({ ruleId: 'arbitration', severity: 'low' })]],
    ]);
    render(
      <PortfolioPanel leases={leases} findingsByLease={findingsByLease} onOpenLease={() => {}} />,
    );
    await gotoMatrix();
  ...
  it('orders rule columns by frequency across leases (most common first)', async () => {
    const leases = [
  ...
  it('shows severity badges in hit cells and em-dash in missing cells', async () => {
    const leases = [meta({ id: 'a', name: 'A.pdf' }), meta({ id: 'b', name: 'B.pdf' })];
    const findingsByLease = new Map<string, Finding[]>([
  ...
  it('shows page count and date in the lease cell', () => {
    ...
    const findingsByLease = new Map<string, Finding[]>([['a', [f({ ruleId: 'r1' })]]]);
    render(
      <PortfolioPanel leases={leases} findingsByLease={findingsByLease} onOpenLease={() => {}} />,
    );
  ...
  it('invokes onOpenLease with the lease id when the lease name is clicked', async () => {
    ...
    const findingsByLease = new Map<string, Finding[]>([['pick', [f({ ruleId: 'r1' })]]]);
    render(
      <PortfolioPanel
        leases={leases}
  ...
  it('uses the first-listed severity if multiple findings share a rule id on one lease', async () => {
    const leases = [meta({ id: 'a', name: 'A.pdf' })];
    const findingsByLease = new Map<string, Finding[]>([
      ['a', [f({ ruleId: 'dup', severity: 'low' }), f({ ruleId: 'dup', severity: 'high' })]],
    ]);
    render(
      <PortfolioPanel leases={leases} findingsByLease={findingsByLease} onOpenLease={() => {}} />,
    );
  ...
  it('marks the lease column as sticky via a data-sticky attribute', async () => {
    const leases = [meta({ id: 'a', name: 'A.pdf' })];
    const findingsByLease = new Map<string, Finding[]>([['a', [f({ ruleId: 'r1' })]]]);
    render(
      <PortfolioPanel leases={leases} findingsByLease={findingsByLease} onOpenLease={() => {}} />,
