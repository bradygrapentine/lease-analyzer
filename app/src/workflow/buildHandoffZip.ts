/**
 * Hand-rolled STORE-only ZIP writer (RFC-compatible enough for
 * every mainstream unzipper). No compression, no external dep.
 *
 * Our inputs are small: a single-lease PDF + a findings JSON/HTML + a
 * README. STORE trades a few tens of KB on disk for zero added deps
 * and a tiny code footprint. See APPNOTE 4.3 for the spec.
 */
...
export function buildHandoffZip(input: HandoffZipInput): Uint8Array {
  ...
  const entries: Entry[] = [
    toEntry('lease.pdf', input.pdfBytes),
    toEntry('findings.html', enc.encode(input.findingsHtml)),
    toEntry('findings.json', enc.encode(input.findingsJson)),
    toEntry('README.txt', enc.encode(input.readme)),
  ];
  ...
  if (cursor !== total) {
    throw new Error(`buildHandoffZip: size mismatch ${cursor} vs ${total}`);
  }
  return out;
}

function toEntry(name: string, data: Uint8Array): Entry {
  return {
    name,
